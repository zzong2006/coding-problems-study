...
for i in range(n):
    for j in range(m - 3):
        answer = max(
            answer, board[i][j] + board[i][j + 1] + board[i][j + 2] + board[i][j + 3]
        )

for i in range(n - 3):
    for j in range(m):
        answer = max(
            answer, board[i][j] + board[i + 1][j] + board[i + 2][j] + board[i + 3][j]
        )
...
for i in range(n - 1):  #
    for j in range(m - 2):
        answer = max(
            answer, board[i][j] + board[i + 1][j] + board[i][j + 1] + board[i][j + 2]
        )
...
for i in range(n - 2):  # ㄱ
    for j in range(m - 1):
        answer = max(
            answer, board[i][j] + board[i + 1][j] + board[i + 2][j] + board[i][j + 1]
        )
...
for i in range(n - 2):
    for j in range(m - 1):
        answer = max(
            answer,
            board[i][j + 1] + board[i + 1][j + 1] + board[i + 1][j] + board[i + 2][j],
        )
