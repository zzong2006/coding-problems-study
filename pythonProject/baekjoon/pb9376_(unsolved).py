            ...
            while que:
                ...
                if how_many < 2:
                    for add_y, add_x in [(-1, 0), (1, 0), (0, 1), (0, -1)]:
                        ...
                        if 0 <= new_y < h and 0 <= new_x < w and board[new_y][new_x] != '*':
                            if board[new_y][new_x] == '#' and memo[new_y][new_x][how_many] > door + 1:
                                memo[new_y][new_x][how_many] = door + 1
                                que.append([new_y, new_x, door + 1, how_many, which])
                            elif board[new_y][new_x] == '.' and memo[new_y][new_x][how_many] > door:
                                ...
                                if (1 << idx) & which != 0 and memo[new_y][new_x][how_many] > door:  # 이미 구출한 인질
                                    memo[new_y][new_x][how_many] = door
                                    que.append([new_y, new_x, door, how_many, which])
                                elif (1 << idx) & which == 0 and memo[new_y][new_x][how_many + 1] > door:
                                    memo[new_y][new_x][how_many + 1] = door
                                    que.append([new_y, new_x, door, how_many + 1, which + (1 << idx)])
            [(y1, x1), (y2, x2)] = hogtie
