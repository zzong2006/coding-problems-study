...
def solution(board):
    def rotate_based_on_axis(ax, ro, clock):
        ...
        if (ax_y == ro_y and ax_x - 1 == ro_x) or (ax_y == ro_y and ax_x + 1 == ro_x):
            # 위로 회전 (시계 방향)
            if clock:
                if 0 <= ro_y - 1 < n and 0 <= ax_y - 1 < n and board[ro_y - 1][ro_x] == 0 and board[ax_y - 1][ax_x] == 0:
        ...
        # 위쪽에 회전이 존재
            ...
            if clock:
                if 0 <= ax_x + 1 < n and 0 <= ro_x + 1 < n and board[ro_y][ro_x + 1] == 0 and board[ax_y][ax_x + 1] == 0:
    ...
    while hq:
        ...
        for i in range(2):
            ...
            if new_pos:
                ...
                if 0 <= new_pos[0] < n and 0 <= new_pos[2] < n and 0 <= new_pos[1] < n and 0 <= new_pos[3] < n \
                        and ((new_pos not in hashmap) or (hashmap[new_pos] > count + 1)) \
                        and (other_new_pos not in hashmap or hashmap[other_new_pos] > count + 1)\
                        and board[new_pos[0]][new_pos[1]] == 0 and board[new_pos[2]][new_pos[3]] == 0:
